...
// centered; click anywhere or press Escape to close. Reads the image
// URL directly from the anchor's href (which survives sanitization).
function _wfGelLightbox(ev, imgUrl, title) {
  ev.preventDefault();
  ...
  hint.style.cssText = 'color:#c8c1b3;font-family:sans-serif;font-size:11px;margin-top:10px;letter-spacing:.04em';
...
/* One global click handler that delegates: any click on .wf-gel-link
   anywhere in the document handles the gel — plain click opens a
   lightbox preview, Cmd/Ctrl+click opens the full Gel Annotation view.
   Runs on script load, not via wfEditorAttach, so read-only workflow
   pages (which never touch the editor) still get the behaviour. */
function _wfInstallGelClickHandler() {
  ...
  document.addEventListener('click', function(e) {
    ...
    if (!link) return;
    var inEditor = !!link.closest('.wf-editor-area');
    var modifier = e.metaKey || e.ctrlKey;
    /* In the editor without a modifier, don't hijack the click — the
       user might be trying to place a cursor next to the link to
       delete/edit it. Modifier click still navigates so the edit-mode
       user can jump to the gel view when they need to. */
    if (inEditor && !modifier) return;
    e.preventDefault();
    if (modifier) {
      // Navigate to Gel Annotation view with this gel selected.
    ...
    // Plain click: lightbox preview using the anchor's own href.
    var imgUrl = link.getAttribute('href');
    if (!imgUrl) return;
    var titleSpan = link.querySelector('span');
    var title = titleSpan ? titleSpan.textContent : '';
    _wfGelLightbox(e, imgUrl, title);
  });
}
// Install now — read-only workflow renders may run before any editor
// is ever attached this session.
...
window.wfEditorAttach = function(el, opts) {
  _wfInjectStyles();
  // (Gel click handler is installed at module load — see the IIFE call
  // below _wfInstallGelClickHandler.)
  opts = opts || {};
