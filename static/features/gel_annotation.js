...
function gelRenderComposite() {
  ...
  exp.width = Math.round(view.w);
  exp.height = Math.round(view.h);
  var ctx = exp.getContext('2d');
  // Draw the cropped sub-rect of the rotated canvas at 1:1.
  ctx.drawImage(rc, view.x, view.y, view.w, view.h, 0, 0, exp.width, exp.height);
  // Overlay: gelDrawOnCtx works in normalised display coords (which,
  // when cropped, exclude annotations outside the crop). We pass the
  // export canvas dims as the "display" size.
...
function gelCropApply() {
  ...
  var d = G.cropDrag;
  // Normalise to display coords 0..1
  var dx0 = Math.min(d.x0, d.x1) / canvas.width;
  var dy0 = Math.min(d.y0, d.y1) / canvas.height;
  var dx1 = Math.max(d.x0, d.x1) / canvas.width;
  var dy1 = Math.max(d.y0, d.y1) / canvas.height;
  dx0 = gelClamp(dx0, 0, 1); dy0 = gelClamp(dy0, 0, 1);
  dx1 = gelClamp(dx1, 0, 1); dy1 = gelClamp(dy1, 0, 1);
...
function gelDrawOnCtx(ctx, w, h, opts) {
  ...
  /* Precompute display-x for each lane (null = outside crop, skip
     drawing AND skip stagger accounting so remaining labels aren't
     pushed into rows they don't need). */
  var laneDispNX = G.lanes.map(function(lane) {
    return gelViewNXtoDispN(lane.x_position);
  });

  /* ── overlap-avoidance: pre-assign each label to a row ──────────────
     Labels used to draw all on row 0 with textAlign=center, so wide
     sample names on adjacent lanes clipped into each other. We now do
     a greedy interval-scheduling pass: for each lane in order, compute
     the label's [x-w/2, x+w/2] interval and place it in the LOWEST row
     where it doesn't overlap anything already there. Row 0 is the top,
     each subsequent row is baseFont + 2 pixels lower. Preserves full
     names — only vertical footprint grows.

     The 4px pad below is a fudge so labels don't visually kiss even
     when their bounding boxes are technically 1px apart. */
  var LABEL_PAD_PX = 4;
  ...
  G.lanes.forEach(function(lane, i) {
    ...
    labelText[i] = label;
    if (laneDispNX[i] === null) return;  // culled by crop
    var textW = ctx.measureText(label).width;
    var x = laneDispNX[i] * w;
    var left = x - textW / 2 - LABEL_PAD_PX;
    var right = x + textW / 2 + LABEL_PAD_PX;
    // Find lowest row where left > rightmost-in-row
    var row = 0;
  ...
  /* draw lanes */
  G.lanes.forEach(function(lane, i) {
    if (laneDispNX[i] === null) return;  // outside crop
    var x = laneDispNX[i] * w;
    ctx.save();
...
function gelInitCanvas() {
  ...
  // Paint the rotated+cropped view into the background canvas. This
  // canvas's intrinsic (attribute) size sets the aspect ratio for the
  // CSS max-width/max-height sizing rule, so the wrap div lays out
  // correctly. Returns false if img isn't loaded yet.
  function paintBg() {
    var rc = gelGetRotatedCanvas();
    if (!rc) return false;
    var view = gelViewRect();
    if (!view || view.w < 1 || view.h < 1) return false;
    bgCanvas.width = Math.round(view.w);
    bgCanvas.height = Math.round(view.h);
  ...
  canvas.onmousemove = function(e) {
    ...
    // Active crop drag: extend the rect to the current cursor.
    ...
    G.lanes[G.dragging].x_position = gelDispNtoViewNX(nx);
    G.dirty = true;
    gelDrawOverlay();
  };
...
function gelInjectStyles() {
  ...
  style.textContent = [
    ...
    /* bg canvas replaces the visible <img> as the sizing element for the
       ...
       produced. display:block + width/height auto lets the intrinsic
       canvas attributes (set by paintBg from the rotated-view dims)
       drive aspect ratio; max-width/max-height scale it to fit. */
    '.gel-bg-canvas { display:block; max-width:100%; max-height:50vh; width:auto; height:auto; user-select:none; background:#000; }',
