...
function _compactCard(p) {
  ...
  var icon = {manual:'&#9998;',file:'&#128196;',paste:'&#128203;',claude:'&#10024;'}[p.source_type] || '&#127760;';
...
function _buildImportUI() {
  return '<div class="proto-import-tabs">' +
    '<button class="proto-tab active" onclick="protoTab(\'claude\',this)">From Claude</button>' +
    '<button class="proto-tab" onclick="protoTab(\'manual\',this)">Write manually</button></div>' +
  '<div id="pt-claude"><div class="proto-import-col">' +
...
function protoTab(name, btn) {
  ...
  ['claude','manual'].forEach(function(t){var el=document.getElementById('pt-'+t);if(el)el.style.display=(t===name)?'':'none';});
...
// ── import actions ────────────────────────────────────────────────────────────
async function protoAddFromClaude() {
  var title=document.getElementById('proto-title-claude')?.value.trim(),
      formatted=document.getElementById('proto-claude-text')?.value;
  if(!title){toast('Add a title',true);return;}
  if(!formatted||!formatted.trim()){toast('Paste Claude\'s formatted output first',true);return;}
  var btn=document.querySelector('#pt-claude .btn.primary');
  if(btn){btn.textContent='Saving...';btn.disabled=true;}
  try{
    var data=await api('POST','/api/protocols/from-claude',{title:title,formatted:formatted,tags:_getTags('import-claude')});
    document.getElementById('proto-title-claude').value='';
    document.getElementById('proto-claude-text').value='';
...
// These timers are NOT tagged with a runId — they're started from the protocol
// *definition* view (browsing, not running), so they aren't scoped to any run
// and won't be removed by Abandon.
