...
async function _hoursLoad() {
  if (!_hoursCategories.length) {
    var catsResp = await api('/api/hours/categories');
  ...
  var resp = await api('/api/hours/entries?start=' + _isoDate(earliest) +
                       '&end=' + _isoDate(latest));
...
async function _hoursOpenEntryModal(id) {
  ...
  if (!entry) {
    ...
    entry = await api('/api/hours/entries/' + id);
  }
...
async function _hoursRenderModal(entry) {
  ...
  var wfSuggest = null;
  try {
    wfSuggest = await api('/api/hours/workflow-day-suggestions?date=' + data.date_iso);
  } catch (e) { /* non-fatal */ }
...
async function _hoursSaveEntry() {
  ...
  try {
    if (_hoursEditingId) {
      await api('/api/hours/entries/' + _hoursEditingId, {
        method: 'PATCH',
        body: JSON.stringify({ start_hour: start, end_hour: end, category: category,
                               notes: notes, workflow_day_date: wfDate || '' })
      });
    } else {
      await api('/api/hours/entries', {
        method: 'POST',
        body: JSON.stringify({
...
async function _hoursDeleteEntry() {
  ...
  try {
    await api('/api/hours/entries/' + _hoursEditingId, { method: 'DELETE' });
    _hoursCloseModal();
    await _hoursLoad();
    _hoursRerender();
    toast('Deleted');
...
async function _hoursRemoveAttachment(attId) {
  ...
  try {
    await api('/api/hours/attachments/' + attId, { method: 'DELETE' });
    await _hoursLoad();
    var updated = _hoursEntries.find(function(e){ return e.id === _hoursEditingId; });
    if (updated) await _hoursRenderModal(updated);
    toast('Removed');
